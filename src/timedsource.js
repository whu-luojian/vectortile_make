...
Timedsource.registerProtocols = function (tilelive) {
	tilelive.protocols['timedsource:'] = Timedsource
}
...
Timedsource.prototype.putTile = function (z, x, y, data, callback) {
	...
	// mvt切片文件输出路径
	var data_output_path = path.join(__dirname, '../data_output/vector/', 'jd' + '.' + z + '.' + x + '.' + y + '.vector.mvt')
	if (!fs.existsSync(data_output_path)) 
		fs.writeFileSync(data_output_path, data)
	callback()
}
